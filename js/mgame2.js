...
function submit1(){
    ...
    if(s == "陈给是猪"){
            window.location.href = "/christmas/chirtmas.html";
    }else{
        $('#modal').iziModal({//初始化
            title: "<font color='white'><b>陈给你是猪吗？你还想蒙混过关？</b></font>",
            headerColor: "#BD5B5B",
            theme: "light",
...
init = function(){
    var pageLeft = 0,
        ...
        getSort = "",
        getBkP="",
        nextCheck = "<button class='nextcheck btn-success'>下一关</button>",
        playAgain = "<button class='playagain btn-info'>再来一次</button>";
    //按下方格触发方法
    $("div[sort]").on("touchstart mousedown",function(e){
        ...
        if(getEvent.changedTouches){//手机情况下
            if(getEvent.changedTouches.length == 1){
                ...
                var getBkimages = $(thisElement).css("backgroundImage");

                var getPoints = parseInt($(thisElement).parents(".piece_container").attr("points"));
                floatLayer = $(document.createElement("div")).attr({"sort": getSort}).css({"backgroundPosition":getBkP, "backgroundImage": getBkimages, "position": "fixed", "width": 300/level,"height": 300/level});//创建浮层并加上样式
    ...
    $(document).on("touchend mouseup",function(e){
        ...
        if(floatLayer){
            ...
            if(timeCount <= 70){
                ...
                $('#modal').iziModal({//初始化
                    title: "<font color='white'><b>'♡♡♡♡♡♡用时'" + $('.rule-description .over-time').text() +"'秒 '+'♡♡♡♡♡♡！请打开你的通关秘籍吧！'</b></font>",
                    headerColor: "#BD5B5B",
                    theme: "light",
                    ransitionInModal: 'transitionIn',
                    transitionOutModal: 'transitionOut',
                ...
                if(!$._data($(".cover_inner button.nextcheck")[0], "events")){//判断是否绑定了事件，如果绑定了，就不再继续绑定
                    $(".cover_inner button.nextcheck").one("touchstart click", function(){
                        ...
                        $(".piece_container div[sort]").css("backgroundImage","url(images/" + points + ".gif)");//去掉上一关卡的样式，加上本关卡的样式
...
$(document).ready(function(){
    ...
    $(".cover_inner button.startGame").on("touchstart click", function(e){
        ...
        clearInterval(timer);
        $(this).parents(".cover").animate({"opacity": 0},2000,function(){$(this).hide();});
        document.getElementById("rule-description").style.display="inline";
